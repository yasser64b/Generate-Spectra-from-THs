...
def spect(Acc, dt):  # acc is in in/s2, dt is time step
    ...
    the spectra for 5% damping and plot them and save them in excel
    file in specific Directory.Have all the txt file for THs a  folder
    """
...
directory = input("Directory you stored THs: ")
SpecType = input("Which spectra are you plotting? SSE or OBE? =")
skip_header = int(input("How many lines in the text file to skip? (It is usually 5) ="))
factor = float(input("Scaling factor  (0.75 for SSE and 0.75/1.5=0.5 for OBE)="))
...
for i in range(0, NumTh, 2):
    ...
    if SpecType == "OBE":
        ...
        DBE_df = pd.DataFrame(
            np.transpose([t, Ax, Ay, freqs]),
            columns=["time", "Spectra_Sa(g)_X", "Spectra_Sa(g)_Y", "Spectra_SRSS(g)"],
        )
...
if SpecType == "OBE":
    ...
    DBE_df_all.to_excel("OBE_spectra.xlsx", index=False)
...
if SpecType == "SSE":
    ...
    MCE_df_all.to_excel("SSE_spectra.xlsx", index=False)
